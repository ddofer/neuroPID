...
aa_groups = ('FYW', 'P', 'C', 'RHK', 'DE', 'VAIL', 'G', 'CSTMNQ', 'R', 'K','RK'
'LVIM','AG','ST','EDNQ','LASGVTIPMC','EKRDNQH')
...
for seq in sampled_proteins :
   ...
   print len(sampled_proteins[seq])
   sampled_proteins[seq].append(length)
   print len(sampled_proteins[seq]), 'length'
   sampled_proteins[seq].append(Z.isoelectric_point())
   print len(sampled_proteins[seq]),'PI'
   sampled_proteins[seq].append(Z.molecular_weight())
   print len(sampled_proteins[seq]),'MW'
   sampled_proteins[seq].append(Z.gravy())
   print len(sampled_proteins[seq]),'gravy'
   sampled_proteins[seq].append(Z.aromaticity())
   print len(sampled_proteins[seq]),'aromaticity'
   sampled_proteins[seq].append(Z.instability_index())
   print len(sampled_proteins[seq]),'instability_index'
   #protparam AA% returns a dict. of K(Aa):V(%) pairs
   sampled_proteins[seq].append(Z.molecular_weight()) #duplicate
   print len(sampled_proteins[seq]),'MW'#duplicate
   sampled_proteins[seq].append(Autocorrellation(sequences[seq],-2))
   print len(sampled_proteins[seq]),'Autocorrellation -2'
   sampled_proteins[seq].append(Autocorrellation(sequences[seq],-3))
   print len(sampled_proteins[seq]),'Autocorrellation -3'
   sampled_proteins[seq].append(aliphaticness(sequences[seq]))
   print len(sampled_proteins[seq]),'aliphaticness'
   # N Glycosylation sites
   NSites = len(re.findall(r'N[^P][ST][^P]', sequences[seq]))
   sampled_proteins[seq].append(NSites / length)
   print len(sampled_proteins[seq]),'N Glycosylation sites'
   # Aspartic acid, asparagine hydroxylation sites
   hydroxSites = len(re.findall('CC.{13}C.{2}[GN].{12}C.C.{2, 4}C',seq))
   sampled_proteins[seq].append(hydroxSites / length)
   print len(sampled_proteins[seq]),'hydroxylation sites'
   #Optional - counts of suspected cleavage sites
   sampled_proteins[seq].append(cleavageCounts(sequences[seq]) / length)
   print len(sampled_proteins[seq]),'counts of suspected cleavage sites'
   y=len(sampled_proteins[seq])
   sampled_proteins[seq].extend(Z.get_amino_acids_percent().values())
   print len(sampled_proteins[seq]),'amino_acids_percent'
   for a in Z.get_amino_acids_percent().keys():
       y=y+1
       print a,y
   sampled_proteins[seq].extend(MerCount(KROne(window_mer)))
   print len(sampled_proteins[seq]),'MerCount KROne'
#combo / mercount length = 32
   sampled_proteins[seq].extend(MerCount(GKROne(window_mer)))
   print len(sampled_proteins[seq]), 'MerCount(GKROne'

   sampled_proteins[seq].extend(MerCount(chargeOne(window_mer)))
   print len(sampled_proteins[seq]), 'MerCount(chargeOne'
   sampled_proteins[seq].extend(seq_Entropy(sequences[seq]))
   print len(sampled_proteins[seq]), 'seq_Entropy'
      #AA Bigrams (400) frequencies:
   sampled_proteins[seq].extend(bigramsFreq(sequences[seq], bigramDict))
   print len(sampled_proteins[seq]), 'bigramsFreq'
   for Aa in aa_groups :
      sampled_proteins[seq].append(PrefixCount(Aa, sequences[seq]))
      print len(sampled_proteins[seq]),Aa,' aa groups Prefix'
      sampled_proteins[seq].append(SuffixCount(Aa, sequences[seq]))
      print len(sampled_proteins[seq]),Aa,' aa groups Suffix'
